...
A Linear-ticket-driven workflow that pairs a git worktree, a tmux session, and a
Claude agent per ticket, with a floating dashboard for navigating between them.

![The ticket dashboard](ticket-dashboard.png)
...
## One ticket → one worktree → one tmux session
